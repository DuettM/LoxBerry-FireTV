...
checks=[('Web-Secret',bool(c.get('web_secret')),'gesetzt','fehlt'),('API-Key',bool(c.get('web_api_key')),'gesetzt','fehlt'),('Config-Rechte',perm_ok,'0600','%04o'%mode),('Private ADB-Ziele',privateonly and not bad_devices,'aktiv','prüfen'),('MQTT Token',tokreq,'aktiv','optional'),('Riskante Befehle',not danger,'gesperrt','freigegeben')]
score=100
...
print('<div class="box" style="margin-top:8px"><b>API-Key für Loxone/Skripte</b><p>Statt des Browser-CSRF-Tokens kann <code>api.cgi</code> mit dem Header <code>X-FireTV-Key</code> oder dem POST-Feld <code>apikey</code> aufgerufen werden. Die htmlauth-Anmeldung bleibt zusätzlich erforderlich.</p><input class="token" type="text" readonly value="%s"><p><label><input type="checkbox" name="rotate_api_key"> Neuen API-Key erzeugen</label></p></div>'%html.escape(str(c.get('web_api_key','')),quote=True))
