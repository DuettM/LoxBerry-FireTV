#!/usr/bin/env python3
import hashlib,hmac,html,ipaddress,json,os,socket,subprocess,sys
...
def q(): return {k:v[-1] if v else '' for k,v in parse_qs(os.environ.get('QUERY_STRING',''),keep_blank_values=True).items()}
...
try:c=json.load(open(CFG,encoding='utf-8'))
 ...
 print('Status: 500 Internal Server Error\r\nContent-Type: text/html; charset=utf-8\r\n\r\n',end='');print(html.escape(str(e)));sys.exit(0)
params=q();scan=params.get('scan')=='1';results=[];net=local_net();known={str(d.get('ip','')) for d in c.get('devices',[])}
if scan and net:
...
print('<footer><small>Fire TV Control · Marco Düthorn · 2026 · v0.2.8</small></footer></div></body></html>')
