...
def adb_info(ip):
 ...
 return {'ip':ip,'model':model,'authorized':authorized,'state':state}
FOLDER=folder();CFG=os.path.join(root(),'config','plugins',FOLDER,'config.json');c=json.load(open(CFG,encoding='utf-8'));f=post_data();scan=False;err='';results=[];net=local_net();known={str(d.get('ip','')) for d in c.get('devices',[])}
...
if results:
 ...
 for r in results:
  already=r['ip'] in known;cls='ok' if r['authorized'] else 'warn';print('<div class="result"><div class="row"><div><b>%s</b><br>%s:5555<br><span class="status %s">%s</span></div>'%(html.escape(r['model']),html.escape(r['ip']),cls,html.escape(r['state'])))
  if already:print('<span class="status ok">Bereits hinzugefügt</span></div></div>')
  else:print('<form class="add" method="post" action="config.cgi"><input type="hidden" name="csrf" value="%s"><input type="hidden" name="form_action" value="add_device"><input type="hidden" name="ip" value="%s"><input type="hidden" name="port" value="5555"><input name="name" maxlength="80" value="%s"><button class="btn green">Hinzufügen</button></form></div></div>'%(html.escape(csrf(c),quote=True),html.escape(r['ip'],quote=True),html.escape(r['model'],quote=True)))
 print('</div></section>')
