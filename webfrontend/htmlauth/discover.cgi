...
if scan and net:
 ...
 order=sorted(found,key=lambda x:tuple(int(p) for p in x.split('.')))
 with ThreadPoolExecutor(max_workers=8) as ex:
  info={ip:ex.submit(adb_info,ip) for ip in order}
 for ip in order:
  try:results.append(info[ip].result())
  except Exception:results.append({'ip':ip,'model':'Fire TV','authorized':False,'state':'error','label':'ADB-Status konnte nicht geprüft werden'})
