...
if os.environ.get('REQUEST_METHOD','GET').upper()=='POST':
 try:
  ...
  if act=='save_general':
   ...
   c.setdefault('devices',[]).append({'id':ident,'name':name,'ip':ip,'port':port,'enabled':True,'cec_on_method':'home','cec_off_method':'sleep'});save(c);notice='Gerät hinzugefügt.' 
...
for d in c.get('devices',[]):
 ident=html.escape(str(d.get('id','')),quote=True);onm=str(d.get('cec_on_method','home'));offm=str(d.get('cec_off_method','sleep'))
