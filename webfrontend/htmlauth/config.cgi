...
SCREEN_METHODS=[('auto','Automatisch (bevorzugt Display Power)'),('display','Display Power: state=ON'),
                ('suspendblocker','mHoldingDisplaySuspendBlocker=true'),('display_or_blocker','Display Power oder Suspend Blocker'),
                ('wakefulness','mWakefulness=Awake')]
def screen_options(c):
 cur=str(c.get('screen_detect','auto'))
 return ''.join('<option value="%s"%s>%s</option>'%(k,' selected' if k==cur else '',html.escape(l)) for k,l in SCREEN_METHODS)
def save(c):
...
if os.environ.get('REQUEST_METHOD','GET').upper()=='POST':
 try:
  ...
  if act=='save_general':
   ...
   if offm not in ('sleep','sleep_repeat','power'):raise ValueError('Ungültige TV-AUS-Methode.')
   d=finddev(c,ident)
