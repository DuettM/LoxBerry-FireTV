...
if os.environ.get('REQUEST_METHOD','GET').upper()=='POST':
 try:
  ...
  if act=='save_general':
   ...
   c['poll_interval']=poll;c.setdefault('mqtt',{})['enabled']=bool(f.get('mqtt_enabled'));c['mqtt']['listen_enabled']=bool(f.get('mqtt_listen'));c['mqtt']['base_topic']=clean_topic(f.get('base_topic'));c.setdefault('watchdog',{})['enabled']=bool(f.get('watchdog_enabled'));save(c);notice='Einstellungen gespeichert.'
  elif act=='add_device':
   ...
   if not 1<=port<=65535:raise ValueError('Port ungültig.')
   ...
   c.setdefault('devices',[]).append({'id':ident,'name':name,'ip':ip,'port':port,'enabled':True});save(c);notice='Gerät hinzugefügt.'
  elif act=='delete':
   ident=f.get('id','') or ''
