...
def form_data():
 ...
 return data
FOLDER=folder();BASE=root();CFG=os.path.join(BASE,'config','plugins',FOLDER,'config.json');BIN=os.path.join(BASE,'bin','plugins',FOLDER)
...
def finddev(c,ident):
 for d in c.get('devices',[]):
  if str(d.get('id',''))==ident:return d
 return None
try:c=load()
...
if os.environ.get('REQUEST_METHOD','GET').upper()=='POST':
 try:
  ...
  if act=='save_general':
   ...
   if not d:raise ValueError('Gerät nicht gefunden.')
   cmd=[os.path.join(BIN,'firetv.py'),'--config',CFG,'--device',ident,'--action','reconnect']
   p=subprocess.run(cmd,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=15);raw=(p.stdout or '').strip();line=raw.splitlines()[-1] if raw else '{}'
   try:r=json.loads(line)
   except Exception:raise ValueError('ADB-Antwort konnte nicht ausgewertet werden: '+raw[-300:])
   state=str(r.get('state',''))
   if r.get('ok'):notice='ADB autorisiert und verbunden.'
   elif state=='unauthorized':notice='ADB-Verbindung neu angefordert. Bitte die Abfrage am Fire TV bestätigen.'
   elif state=='offline':error='Fire TV ist per ADB offline. ADB-Debugging bzw. Netzwerk prüfen.'
   else:error=str(r.get('message') or 'ADB-Verbindung konnte nicht hergestellt werden.')
  elif act=='delete':
   ident=f.get('id','') or '';c['devices']=[d for d in c.get('devices',[]) if str(d.get('id',''))!=ident];save(c);notice='Gerät gelöscht.'
