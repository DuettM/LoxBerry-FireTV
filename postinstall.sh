...
PCGI="${LBPCGI:?LBPCGI missing}/$PDIR"
PHTML="${LBPHTML:?LBPHTML missing}/$PDIR"
mkdir -p "$PCONFIG" "$PLOG"
if [ ! -f "$PCONFIG/config.json" ]; then cp "$PCONFIG/config.default.json" "$PCONFIG/config.json" || exit 2; fi
...
chmod 755 "$PBIN/firetv.py" "$PBIN/mqtt_listener.py" "$PBIN/watchdog.py" "$PBIN/secure_update.py" 2>/dev/null || true
chmod 755 "$PCGI/index.cgi" "$PCGI/config.cgi" "$PCGI/debug.cgi" "$PCGI/api.cgi" "$PHTML/firetv.cgi" 2>/dev/null || true
...
echo "<OK> Fire TV Control $PVERSION installiert. CGI-Rechte wurden gesetzt."
exit 0
