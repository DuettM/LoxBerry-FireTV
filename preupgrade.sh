...
if [ -f "$PCONFIG/config.json" ]; then
  if python3 -m json.tool "$PCONFIG/config.json" >/dev/null 2>&1; then
    ...
    echo "<INFO> Fire TV user configuration backed up from $PCONFIG/config.json."
  else
    echo "<WARNING> Existing Fire TV configuration is invalid JSON; refusing to overwrite backup."
  fi
else
  echo "<WARNING> No existing Fire TV config.json found at $PCONFIG."
fi
exit 0
